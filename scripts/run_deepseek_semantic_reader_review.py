...
def prompt_for(chapter: str) -> tuple[str, str, list[Path]]:
    ...
    system = (
        "You are DeepSeek acting as an independent semantic reader reviewer for fiction. "
        "You only inspect the supplied official chapter, brief, context pack, review context, style contract, reader promise, and derived ledgers. "
        "Do not read Codex review files, Codex semantic reports, anti-AI reports, ai_taste reports, dialogue_function reports, model disagreement, or aggregate semantic_reader_review files. "
        "Do not modify repository files. Return one JSON object only."
    )
    user = "\n\n".join(
        [
            f"Review chapter {chapter} for semantic reader-experience risk.",
            "Use real reading judgment, not keyword counting.",
            ...
            "Return JSON with keys: status, action, summary, categories, scene_samples, blockers, warnings.",
...
def extract_json_object(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        if start == -1 or end == -1 or end <= start:
            raise ValueError("DeepSeek semantic reader review did not return a JSON object") from None
        data = json.loads(stripped[start : end + 1])
    if not isinstance(data, dict):
        raise ValueError("DeepSeek semantic reader review JSON must be an object")
    return data
...
def normalize_category(key: str, value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        return {
            ...
            "revision_actions": [f"Regenerate the DeepSeek semantic reader review with category {key} filled."],
        }
...
def normalize_scene_samples(value: Any) -> list[dict[str, str]]:
        ...
        samples.append(
            {
                ...
                "reader_effect": str(item.get("reader_effect", "")).strip(),
                "drama_movement": str(item.get("drama_movement", "")).strip(),
...
def normalize_report(parsed: dict[str, Any], chapter: str, model: str, inputs: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "scene_samples": normalize_scene_samples(parsed.get("scene_samples")),
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for sample in report.get("scene_samples", []):
    ...
    lines = [
        f"# DeepSeek Semantic Reader Review: {report.get('chapter')}",
        "",
    ...
    if isinstance(categories, dict):
        for key in CATEGORIES:
            ...
            lines.extend(f"  - {quote}" for quote in item.get("evidence_quotes", []) or ["none"])
...
def print_not_ready(message: str) -> None:
    print("# DeepSeek Semantic Reader Review")
    print()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ask DeepSeek for an independent semantic reader review.")
    parser.add_argument("--chapter", required=True)
    parser.add_argument("--model", default=model_for("deepseek_semantic_reader_review"))
    ...
    if write_blocked_by_locks("DeepSeek semantic reader review"):
        return 1
    try:
    ...
    prompt_path = run_dir / "semantic_reader_review.prompt.md"
    raw_path = run_dir / "semantic_reader_review.raw.json"
    out_json = ROOT / "reviews" / args.chapter / "deepseek_semantic_reader_review.json"
    out_md = ROOT / "reviews" / args.chapter / "deepseek_semantic_reader_review.md"
    write_text(prompt_path, f"# System\n\n{system}\n\n# User\n\n{user}\n")
    if args.dry_run:
        write_run_manifest(
            chapter=args.chapter,
            kind="semantic_reader_review",
            model=args.model,
            dry_run=True,
            prompt_path=prompt_path,
            input_paths=inputs,
            isolation_attestation="Dry run only wrote the DeepSeek semantic reader review prompt and did not read Codex review files or semantic aggregate reports.",
        )
    ...
    try:
        ...
        print(f"ERROR: invalid DeepSeek semantic reader review response: {exc}", file=sys.stderr)
        return 1
    ...
    write_run_manifest(
        ...
        isolation_attestation="DeepSeek semantic reader review was not given Codex reviews, Codex semantic reports, anti-AI reports, ai_taste, dialogue_function, model disagreement, or aggregate semantic_reader_review files.",
    )
