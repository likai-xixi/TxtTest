...
- `章节标题` 必须是非占位标题；候选正文最终也必须以 `# 标题` 开头。
- `本章进展契约` 必须包含：进展类型、有效推进类型、有效推进单位（before -> after）、有效推进证据目标、推进对象、起始状态依据、结束状态变化、最低落账事件、进展重要度、低牵引功能；最低落账事件必须使用现有 event ledger 类型。
...
- `本章留存合同` 必须包含 reader_reward_intensity、reader_reward_type、reader_reward_delivery、reader_reward_timing、reward_evidence_requirement、低戏剧载体、低戏剧载体承载的推进类型、核心机制是否出现、若未出现当前沉默计数、等待结尾债务；R 档必须来自 reader promise 的手动策略，不得自行默认。
