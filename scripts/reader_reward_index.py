...
from _common import ROOT, chapter_parts, now_iso, read_json, write_json
...
def build_index() -> dict[str, Any]:
    ...
    for path in gate_files():
        ...
        ineffective_low = bool(low_carrier and low_carrier not in {"none", "无"} and low_progress not in EFFECTIVE_PROGRESS_TYPES)
        if ineffective_low and low_carrier == previous_low_carrier:
            low_drama_run += 1
        elif ineffective_low:
            low_drama_run = 1
        else:
            low_drama_run = 0
        previous_low_carrier = low_carrier if ineffective_low else ""
        low_limit = int(rules.get("low_drama_repeat_max_run") or 999)
        if low_drama_run > low_limit:
